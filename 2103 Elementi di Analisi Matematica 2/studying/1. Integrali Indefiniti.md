...
#### Caso 2: $\frac{1}{(x^{2}+px+q)^{n}}\quad p,q \in \mathbb{R}: \Delta = p^{2}-4q<0,\, n \in \mathbb{N}$
> [!hint] Metodo risolutivo
...
> Si procede quindi effettuando moltiplicazioni o somme di costanti e si procede utilizzando i vari metodi risolutivi sopra citati.
### Integrali di Equazioni Razionali Fratte
$$ \int \frac{N(x)}{D(x)} \,dx$$
#### grado $[N(x)]$ $\geq$ grado $[D(x)]$
> [!hint] Metodo risolutivo
> È sufficiente effettuare la divisione tra polinomi finché non si ottiene un polinomio di grado inferiore al denominatore.

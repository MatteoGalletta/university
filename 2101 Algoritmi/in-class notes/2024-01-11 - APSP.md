...
Valutando $V_{1}$, deve esistere un cammino diretto che collega $s$ e $t$, oppure un nodo intermedio che collega $s$ e $t$.
Valutando ${} V_{2} {}$, deve esistere un cammino diretto che collega $s$ e $t$, oppure due nodi intermedi che collegano $s$ e $t$.
...
...
La seguente implementazione prevede l'utilizzo di un'unica matrice `D`:
```
FLOYD-WARSHALL(w, n):
